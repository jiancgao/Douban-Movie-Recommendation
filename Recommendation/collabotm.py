...
from Recommendation.simplex_projection import euclidean_proj_simplex
from Recommendation.formatted_logger import formatted_logger

logger = formatted_logger('CollaborativeTopicModel', 'info')

e = 1e-100
error_diff_ratio = 0.01
...
class CollaborativeTopicModel:
    ...
    def __init__(self, n_topic, n_voca, n_user, n_item, verbose=True):
    ...
    def set_doc(self, doc_ids, doc_cnt):
        self.doc_ids = doc_ids
        self.doc_cnt = doc_cnt

    def set_ratings(self, user, item, rate):
        self.C[user, item] += self.a - self.b
        self.R[user, item] = rate
    ...
    def mean_sqr_error(self):
        ...
        err = err.sum() / (self.n_item * self.n_user)
        return err
